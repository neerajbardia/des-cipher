def main():
    ...
    key = input("Enter a key of 8 length (64-bits) (characters or numbers only) : ")        #Main Function for I/O
    print()
...
eachRoundPermutationMatrix = [			#this matrix is used for the permutation process of each step
...
finalPermutationMatrix = [					#this matrix is going to be used for the final permutation
...
def DESEncryption(key, text, padding):			#this is the encryption function
...
def DESDecryption(key, text, padding):			#this is the decryption function
...
initialPermutationMatrix = [			#this is the matrix for the initial permutation of the data
...
expandMatrix = [				#this matrix is very imp for the process to apply the actual xor
    32, 1, 2, 3, 4, 5,
    4, 5, 6, 7, 8, 9,
    8, 9, 10, 11, 12, 13,
    12, 13, 14, 15, 16, 17,
...
keyPermutationMatrix1 = [     #this is the permutation matrix for the KEY
    57, 49, 41, 33, 25, 17, 9,
    1, 58, 50, 42, 34, 26, 18,
    10, 2, 59, 51, 43, 35, 27,
    19, 11, 3, 60, 52, 44, 36,
...
def SboxSubstitution(bitArray):
    """Function to substitute all the bytes using Sbox."""		#Sbox Function
...
def addPadding(text):
    """Function to add padding according to PKCS5 standard."""			#Function to Add Padding Values
...
def removePadding(data):													#Function to remove Padding Value
...
def expand(array, table):
    """Function to expand the array using table."""                                #Function of Expand
...
def permutation(array, table):
	...
	# Returning permuted result														#Make Permutation		
    return [array[element - 1] for element in table]

def leftShift(list1, list2, n):
    """Function to left shift the arrays by n."""
    # Left shifting the two arrays													#Function for LeftShift 	
    return list1[n:] + list1[:n], list2[n:] + list2[:n]

def nSplit(list, n):
    """Function to split a list into chunks of size n."""
    # Chunking and returning the array of chunks of size n							#Splitting of an Array
    # and last remainder
...
def xor(list1, list2):
    ...
    return [element1 ^ element2 for element1, element2 in zip(list1,list2)]				#Function for Performing XOR

def binValue(val, bitSize):
    """Function to return the binary value as a string of given size."""					#Function for Converting into binary
...
def stringToBitArray(text):
    """Funtion to convert a string into a list of bits."""					#Function for Converting String to Binary
...
def bitArrayToString(array):
    ...
    # Initializing variables required
    stringBytesList = []												#Binary value to String
    stringResult = ''
    # For each byte
